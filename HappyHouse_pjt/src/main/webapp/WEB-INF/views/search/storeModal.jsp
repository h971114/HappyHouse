...
<h3>주변 편의시설</h3>
		<div style="text-align: right; margin:10px 0px;">
			<button id="return" class="btn btn-primary">취소</button>
		</div>
		<script type="text/javascript">
			$(document).ready(function(){
				...
				$(".page-item").click(function() {
					...
					$("#pageform").attr("action", "${root}/ssafy/storemodal?dong=${dong}&").submit();
		...
		<table class="table">
			<thead class="thead-light">
				<tr>
					<td style="width:30%">상가 이름</td>
					<td style="width:30%">지점명</td>
					<td style="width:30%">업종명</td>
				</tr>
			</thead>
			<tbody>
			...
			<c:forEach items="${list}" var="store">
				...
				<td style="width:30%">${store.storename}</td>
				<td style="width:30%">${store.storeloc}</td>
				<td style="width:30%">${store.storecodename}</td>
			</tr>
			</c:forEach>
			</tbody>
		</table>
